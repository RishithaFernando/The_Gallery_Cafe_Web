...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['update_status'])) {
        ...
        if (mysqli_query($conn, $sql)) {
            echo "<script>alert('Order status updated successfully!'); window.location.href = 'view_order_staff.php';</script>";
        } else {
            echo "<script>alert('Error: " . mysqli_error($conn) . "'); window.location.href = 'view_order_staff.php';</script>";
        }
        exit();
    }
}
...
<body>
    ...
    <table>
        ...
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '<td>';
                echo '<form method="post" action="view_order_staff.php" style="margin: 0;">';
    ...
    <a href="../html/staff.html" class="back-link">Back to  Dashboard</a>
</body>
</html>
